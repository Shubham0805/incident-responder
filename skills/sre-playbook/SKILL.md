...
Compare the `current_dsn` against `last_known_good_dsn` yourself — do not
just trust a single field. Look specifically for things like a mistyped
hostname, wrong port, wrong credentials, or pool-size change. Print a short
summary from your script; you don't need to dump raw JSON into the
conversation.
...
connection attempt fails to resolve and the pool is exhausted"). If the
evidence doesn't clearly support a single root cause, say what's still
